...
function analyzeTechnicalStateless(candidate: CandidateSignal, candles: OHLCV[]): TechnicalOpportunity | null {
  ...
  const trendAligned =
    snap == null ||
    (direction === "BUY" ? (snap.ema9 >= snap.ema20 && entry > snap.ema9) : (snap.ema9 <= snap.ema20 && entry < snap.ema9));

  // Require meaningful trend strength — ADX below 20 means no trend
  const hasTrendStrength = snap == null || snap.adx14 >= 20;
  ...
  // The base logic requires strict trend alignment for momentum
  if (!trendAligned) return null;
  if (!hasTrendStrength) return null;
  
  if (snap) {
    // Hard RSI confirmation for momentum
    if (direction === "BUY" && snap.rsi14 < 55) return null;
    if (direction === "SELL" && snap.rsi14 > 45) return null;
  }
