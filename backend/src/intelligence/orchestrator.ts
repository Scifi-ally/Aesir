...
class ScannerOrchestrator {
  ...
  private readonly telemetry = new TelemetryEngine();
  ...
  private currentDecayTelemetry: ModelDecayTelemetry | null = null;
  ...
  getSnapshot(): IntelligenceSnapshot {
    return {
      ...
      compositeContext: getCompositeMarketContext(),
      ...
      this.frontendTimer = setInterval(() => {
        ...
        broadcast(createServerEvent.marketIntelligenceUpdate({
          snapshot: {
            ...
            compositeContext: snapshot.compositeContext,
            startedAt: snapshot.startedAt,
            updatedAt: snapshot.updatedAt,
          },
          modelDecayTelemetry: this.currentDecayTelemetry ?? undefined,
          topMovers,
    ...
    if (!this.breadthTimer) {
      ...
      setInterval(async () => {
        try {
          ...
          if (telemetry) {
            this.currentDecayTelemetry = telemetry;
          }
        } catch (e) {
