...
// Wait for database to be ready (Postgres takes a moment to start or recover)
let dbReady = false;
for (let i = 0; i < 15; i++) {
  try {
...
if (!dbReady) {
  logger.fatal("CRITICAL: Could not connect to the database after 15 attempts. Is Postgres running?");
  process.exit(1);
}
