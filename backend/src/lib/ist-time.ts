...
// Whole calendar days from `fromStr` to `toStr` (both YYYY-MM-DD in IST).
// Positive when `toStr` is later. Used to measure how far a data feed's
// freshest bar lags the trading day we expect it to cover.
export function istDaysBetween(fromStr: string, toStr: string): number {
  const from = parseISTDate(fromStr).getTime();
  const to = parseISTDate(toStr).getTime();
