...
// Directional-regime hysteresis band (niftyChangePct, in %). While a trend
// regime stands, the trend reading is nudged this far in the trend's direction
// before a boundary flip is allowed — kills tick-to-tick regime thrash near the
// ±0.2 / ±0.5 boundaries. See Step 4.
const REGIME_HYSTERESIS_PCT = 0.1;
...
export function detectRegime(): void {
  ...
  if (niftyChangePct === null || indiaVix === null) {
    if (isInitialGrace) {
      ...
      // Force deterministic classification with NEUTRAL values
      // Now explicit default to UNKNOWN per Phase 4 requirements
      inputsForced = true;
  ...
  // If we had to fabricate inputs (e.g. weekend/no data), safely default to UNKNOWN
  if (inputsForced) {
    _lastRegimeOutput = {
      regime: "UNKNOWN",
      confidence: 0,
      ...
      sectorBreadth: 50,
      suggestionsPaused: false,
      pauseReason: "Live data unavailable — defaulting to UNKNOWN regime",
      timestamp: new Date().toISOString(),
      decisionReason: "Live data unavailable (inputs forced)",
      inputsForced: true,
    };
    updateMarketState({ regime: "UNKNOWN", suggestionsPaused: false, pauseReason: null });
    return;
  }
  ...
  // Directional hysteresis: bias the trend reading toward the standing regime
  // so a niftyChangePct hovering near a boundary (±0.2 / ±0.5) doesn't flip the
  // regime every tick. Mirrors the VIX pause latch — one state variable
  // (previousRegime) widens the exit boundary in both directions: while
  // TRENDING_UP the reading is nudged up (harder to fall to SIDEWAYS, harder
  // still to cross into bearish) and vice versa. Confidence still uses the raw
  // change so its magnitude stays honest.
  else if (niftyChangePct !== null) {
    const trendBias =
      previousRegime === "TRENDING_UP" ? REGIME_HYSTERESIS_PCT
      : previousRegime === "TRENDING_DOWN" ? -REGIME_HYSTERESIS_PCT
      : 0;
    const nHyst = niftyChangePct + trendBias;
    if (nHyst > 0.5) {
