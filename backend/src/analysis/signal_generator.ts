...
export interface IntelligenceSignal {
  ...
  target2: number;
  riskReward: number;
  // Expected value in R after costs: p·RR − (1−p) − costR. Present only when the
  // learned ranker served a calibrated P(win); null under graceful degradation.
  // This — not the 0-100 confidence blend — is the primary ranking key.
  evR: number | null;
...
export async function runIntelligencePipeline(
  ...
  for (const candidate of candidates) {
    ...
    if (rankerLoaded && typeof winProb === "number") {
      ...
      // ── Expected value in R, after costs — the true ranking key ──────────
      // A high P(win) on a thin-payoff setup can still be a losing bet once
      // costs bite; a modest P(win) on a fat-payoff setup can be the better
      // trade. Rank and gate on EV, not the confidence blend.
      //   EV_R = p·RR − (1−p)·1 − costR
      // costR = round-trip cost fraction / stop-distance fraction, i.e. how
      // many R the ~2× per-side round trip eats given this stop width. Sourced
      // from lib/costs so EV gating shares one cost basis with outcome scoring.
      const entry = result.setup.entryPrice || snap.close;
      const stop = result.setup.stopLoss || entry;
      const rr = result.setup.riskReward || 0;
      const stopFrac = entry > 0 ? Math.abs(entry - stop) / entry : 0;
      const costR_ = costR(stopFrac);
      evR = winProb * rr - (1 - winProb) - costR_;

      // Gate: refuse non-positive expected value even when P(win) cleared the
      // ranker threshold. This is what the 0.58 gate alone cannot see.
      if (!(evR > 0)) {
        rejectedByAI++;
        rejectedSignals.push(buildRejectedSignal("negative_ev", Math.round(evR * 1000) / 1000, 0));
        logger.debug(
          { symbol: result.symbol, winProb, rr, costR: costR_, evR, setupType: result.setup.setupType },
          "Signal rejected — non-positive expected value in R after costs",
        );
        continue;
      }
    }
  ...
  // Rank by expected value in R (after costs) — the true edge — descending.
  // Signals with a computed evR always rank above those without (graceful
  // degradation: no ranker → evR null → fall back to the confidence blend).
  // Confidence breaks ties among equal-EV or both-null signals.
  signals.sort((a, b) => {
    const aHas = typeof a.evR === "number";
    const bHas = typeof b.evR === "number";
    if (aHas && bHas) {
