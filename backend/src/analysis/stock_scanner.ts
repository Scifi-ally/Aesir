...
import { getISTDateStr, getLastCompletedTradingDayStr, shiftISTDateStr, istDaysBetween } from "../lib/ist-time";
...
export async function scanStock(
  ...
  try {
    ...
    // Phase 4: volume-freshness guard. volumeRatio compares the freshest daily
    // bar's volume to the prior-20 average; if the feed lags, that "surge" is
    // measured on a stale day and the momentum/volume gates fire on old data.
    // When the freshest bar trails the day we expect by more than a long
    // weekend + holiday (4 calendar days), neutralize the ratio to 1.0 so every
    // surge gate fails safe rather than trusting a stale spike. Keyed off the
    // as-of date so backtests (toDateOverride set) are unaffected by wall clock.
    const asOf = toDateOverride ?? getLastCompletedTradingDayStr();
    const freshestBar = dailyCandles[dailyCandles.length - 1]!;
    const barDateStr = getISTDateStr(new Date(freshestBar.timestamp));
    const lagDays = istDaysBetween(barDateStr, asOf);
    if (lagDays > 4 && snap.volumeRatio !== 1) {
      logger.warn(
        { symbol: stock.symbol, barDate: barDateStr, asOf, lagDays, volumeRatio: snap.volumeRatio },
        "scanStock: daily volume bar stale — neutralizing volumeRatio to 1.0",
      );
      snap.volumeRatio = 1;
    }
