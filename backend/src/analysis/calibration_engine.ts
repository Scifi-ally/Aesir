...
export interface SetupCalibration {
  ...
  // How many winners contributed to avgAdverseExcursionPct — gates stop-tightening
  // (moving a real stop off a 2-winner average is noise, not evidence).
  adverseExcursionSamples: number;
    ...
    try {
      ...
      for (const [k, b] of buckets) {
        next.set(k, {
          ...
          adverseExcursionSamples: b.maeOnWinCount,
...
// ── Empirical stop tightening (Phase 2) ───────────────────────────────────────
// Winners' realized MAE tells us how much adverse room a winning trade actually
// needed. A stop wider than that is dead capital: it never saved a winner, it
// only inflated R (shrinking the reward-multiple and, downstream, Kelly size).
// We tighten the stop toward realized MAE — never wider, never past a per-call
// cap, and only with enough winner samples to trust the number.
export const EMPIRICAL_STOP_MIN_SAMPLES = 12; // winner-MAE samples, not total trades
const MAE_BUFFER = 1.5;    // give winners 1.5× their average adverse move of room
const MAX_TIGHTEN_FRAC = 0.5; // never cut more than half the current stop distance in one pass

export interface EmpiricalStopResult {
  stop: number;
  adjusted: boolean;
  reason: string;
}
...
/**
 * Suggest a stop tightened toward realized winner-MAE. Pure — no I/O.
 * Returns the original stop unchanged (adjusted:false) when evidence is thin,
 * MAE is missing/non-positive, or the empirical stop wouldn't be tighter.
 */
export function empiricalStop(
  entry: number,
  currentStop: number,
  isBuy: boolean,
  cal: Pick<SetupCalibration, "avgAdverseExcursionPct" | "adverseExcursionSamples"> | null,
): EmpiricalStopResult {
  const currentDist = Math.abs(entry - currentStop);
  if (!cal || cal.avgAdverseExcursionPct == null || cal.avgAdverseExcursionPct <= 0) {
    return { stop: currentStop, adjusted: false, reason: "no-mae" };
  }
  if (cal.adverseExcursionSamples < EMPIRICAL_STOP_MIN_SAMPLES) {
    return { stop: currentStop, adjusted: false, reason: `thin-samples(${cal.adverseExcursionSamples})` };
  }
  if (!(entry > 0) || !(currentDist > 0)) {
    return { stop: currentStop, adjusted: false, reason: "bad-inputs" };
  }

  const buffered = cal.avgAdverseExcursionPct * entry * MAE_BUFFER;
  const floor = currentDist * (1 - MAX_TIGHTEN_FRAC);
  const newDist = Math.max(buffered, floor);
  if (newDist >= currentDist) {
    return { stop: currentStop, adjusted: false, reason: "already-tight" };
  }
  const stop = isBuy ? entry - newDist : entry + newDist;
  return {
    stop,
    adjusted: true,
    reason: `mae=${(cal.avgAdverseExcursionPct * 100).toFixed(2)}% n=${cal.adverseExcursionSamples} ${currentDist.toFixed(2)}->${newDist.toFixed(2)}`,
  };
}
