...
import { calibrateConfidence, empiricalStop, isSetupDemoted, isSetupDemotedForRegime } from "../analysis/calibration_engine";
...
export async function ingestSignal(
  ...
  // Phase 2: pull the stop in to where winners of this setup actually live.
  // A stop wider than winners' realized adverse excursion never saved a winner
  // — it only bleeds R on losers. empiricalStop only ever tightens, never
  // widens, and is gated on ≥EMPIRICAL_STOP_MIN_SAMPLES winner-MAE samples.
  const es = empiricalStop(
    signal.entryPrice,
    signal.stopLoss,
    signal.signal === "BUY",
    empirical,
  ...
  if (es.adjusted) {
    const newDist = Math.abs(signal.entryPrice - es.stop);
    signal.stopLoss = es.stop;
    signal.stopDistancePct = (newDist / signal.entryPrice) * 100;
    // positionSize is held fixed so notional never grows and the capacity gates
    // below see the same size they always would; only realized risk shrinks.
    signal.maxRiskInr = signal.positionSize * newDist;
    // riskReward was set on the live (ltp) basis in the recheck above; recompute
    // on the same basis so the tighter risk is reflected truthfully.
    const rewardToT1 =
      signal.signal === "BUY" ? signal.target1 - ltp : ltp - signal.target1;
    const liveRisk = Math.abs(ltp - signal.stopLoss);
    if (liveRisk > 0 && rewardToT1 > 0) {
      signal.riskReward = Number((rewardToT1 / liveRisk).toFixed(2));
    }
    logger.info(
      {
        symbol: signal.symbol,
        ...
        maxRiskInr: signal.maxRiskInr,
        riskReward: signal.riskReward,
        reason: es.reason,
      },
      "Stop tightened to empirical winner-MAE",
    );
  } else {
    logger.debug(
      { symbol: signal.symbol, setupType: signal.setupType, reason: es.reason },
      "Empirical stop-tighten skipped",
    );
  }
