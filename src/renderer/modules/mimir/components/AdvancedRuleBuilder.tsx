...
import { FADE_FAST, SPRING_SNAPPY } from "@/modules/mimir/lib/motion";
    ...
    <div className="mx-auto flex w-full max-w-[610px] shrink-0 flex-col gap-1 px-1 pb-1">
      ...
      <div className="flex items-center justify-between pt-1 pb-1 px-2">
        ...
        <button type="button" onClick={handleSave} disabled={createRuleMutation.isPending} className="group flex h-10 items-center gap-2 rounded-xl bg-primary px-5 text-xs font-normal text-primary-foreground transition-all hover:opacity-90 disabled:opacity-50 active:scale-[0.98]">
          ...
          {createRuleMutation.isPending ? <div className="h-3.5 w-3.5 animate-spin rounded-full border-2 border-current border-t-transparent" /> : <ArrowRight className="h-3.5 w-3.5 transition-transform group-hover:translate-x-0.5" />}
        </button>
      </div>
    </div>
  );
