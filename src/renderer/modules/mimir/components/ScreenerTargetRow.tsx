...
import { prefetchSymbol } from "@/modules/mimir/lib/prefetch";
import { useStore } from "@/modules/mimir/store/useStore";
import type { ScreenerTarget } from "@/modules/mimir/hooks/useScreener";
    ...
    >
      ...
      <div className="flex flex-col justify-center gap-1 min-w-0 flex-1 z-10 pointer-events-none">
        ...
        <div className="flex flex-wrap items-center gap-1.5 min-w-0">
          {badges.map((badge) => (
            <span key={badge} className="text-[11px] font-normal uppercase tracking-wider text-foreground/70 truncate">
              {badge}
            </span>
          ))}
          {badges.length === 0 && (
