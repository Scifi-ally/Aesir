...
import { Tooltip } from "@/modules/mimir/components/mimir/tooltip";
import { FADE_SLOW, FADE_STANDARD } from "@/modules/mimir/lib/motion";
...
export function SettingsDialog({ isOpen, onClose }: SettingsDialogProps) {
  ...
  const renderField = (
    ...
    const inputId = `settings-field-${label.replace(/\s+/g, '-').toLowerCase()}`;

    return (
      <div className="flex flex-col group">
        <div className="flex items-center gap-2 mb-1.5">
          <label htmlFor={inputId} className="text-sm font-normal text-foreground tracking-tight group-focus-within:text-primary transition-colors">{label}</label>
        ...
        <form className="relative" onSubmit={(e) => e.preventDefault()}>
          ...
          <input
            ...
            spellCheck={false}
            aria-label={label}
            className="w-full bg-transparent text-sm font-mono text-foreground outline-none ring-0 border-0 border-b border-foreground/10 focus:border-primary hover:border-foreground/25 px-0 py-2.5 transition-colors shadow-none rounded-none placeholder:text-muted-foreground/30 focus:bg-primary/5"
          ...
          {isSecret && secretKey && (
            <button
              ...
              aria-label={showSecrets[secretKey] ? `Hide ${label}` : `Show ${label}`}
              className="absolute right-0 top-1.5 text-[10px] font-normal text-muted-foreground hover:text-foreground focus:text-primary transition-colors outline-none ring-0 border border-foreground/10 hover:border-foreground/20 focus:border-primary/50 rounded px-2 py-1 uppercase tracking-[0.1em]"
            >
        ...
        <>
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row flex-1 overflow-hidden">
              ...
              <div className="flex sm:flex-col shrink-0 gap-1 px-4 pt-3 pb-1 sm:w-48 sm:px-6 sm:pt-8 sm:pb-0 overflow-x-auto sm:overflow-x-visible sm:overflow-y-auto no-scrollbar whitespace-nowrap">
                {TAB_ITEMS.map((tab) => (
                  <button
                    key={tab.key}
                    onClick={() => setActiveTab(tab.key)}
                    className={cn(
                      "text-[13px] font-medium text-left transition-all py-2.5 px-3 rounded-lg shrink-0",
                      activeTab === tab.key
