...
import { calculateSRLevels, type SRLevel } from "@/modules/mimir/lib/technicalAnalysis";
...
export const SupportResistancePanel = React.memo(function SupportResistancePanel({
  ...
  const livePrevClose = useSymbolDataSelector(selectedSymbol, d => d.prevClose);

  const fallbackLevels = useMemo(() => {
    if (!currentPrice) return null;
    const c = livePrevClose || currentPrice;
    const h = currentPrice > c ? currentPrice * 1.008 : c * 1.008;
    const l = currentPrice < c ? currentPrice * 0.992 : c * 0.992;
    const p = (h + l + c) / 3;
    const r1Price = 2 * p - l;
    const r2Price = p + (h - l);
    const s1Price = 2 * p - h;
    ...
    const makeSR = (label: string, price: number, type: "support" | "resistance"): SRLevel => ({
      label,
      price,
      type,
      strength: 5,
      sources: ["Pivot Fallback"],
    });

    return {
      r1: makeSR("R1", r1Price, "resistance"),
      r2: makeSR("R2", r2Price, "resistance"),
      s1: makeSR("S1", s1Price, "support"),
      s2: makeSR("S2", s2Price, "support"),
    };
  }, [currentPrice, livePrevClose]);

  // Map levels array to labeled pivot levels (real computed values only)
  const levels = useMemo(() => {
    if (!baseLevels || !Array.isArray(baseLevels)) return fallbackLevels;

    const findByLabel = (lbl: string): SRLevel | undefined => baseLevels.find(l => l.label === lbl);
    return {
      r1: findByLabel("R1") || fallbackLevels?.r1,
      r2: findByLabel("R2") || fallbackLevels?.r2,
      s1: findByLabel("S1") || fallbackLevels?.s1,
      s2: findByLabel("S2") || fallbackLevels?.s2,
    };
  }, [baseLevels, fallbackLevels]);
