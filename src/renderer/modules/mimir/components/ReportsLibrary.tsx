...
const borderlessMarkdownComponents = {
  ...
  h3: ({ children }: any) => (
    <div className="flex items-center gap-2.5 mt-8 mb-3 pb-1 border-b border-border/10">
      ...
      <h3 className="text-xs font-mono tracking-wider uppercase text-muted-foreground font-bold m-0">{children}</h3>
    </div>
  ),
  table: ({ children }: any) => (
  ...
  thead: ({ children }: any) => (
    <thead className="border-b border-border/15 text-[10px] uppercase font-mono tracking-wider text-muted-foreground">{children}</thead>
  ),
  tr: ({ children }: any) => (
  ...
  blockquote: ({ children }: any) => (
    <div className="my-6 pl-4 border-l-2 border-muted-foreground py-1.5 text-foreground/90 font-mono text-sm flex items-center gap-3">
