...
import { ArrowRight, ShieldCheck, KeyRound, Phone } from "lucide-react";
...
export function UpstoxHeadlessLogin({ type, onSuccess }: Props) {
  ...
  // Backend automatically cleans up old sessions on next begin().
  // We don't call api.headlessAuth.cancel() on unmount to avoid React StrictMode 
  // double-mount destroying the active session.
    ...
    <div className="flex flex-col w-full h-full relative" style={{ padding: "0 8px 16px 8px" }}>
      <AnimatePresence mode="wait">
        {step === "detecting" && (
          ...
          >
            <div className="w-6 h-6 rounded-full border-2 border-bull border-t-transparent animate-spin" />
        ...
        {step === "phone" && (
          ...
          >
            ...
            <button type="submit" disabled={isProcessing || phone.length < 10} className={btnClass}>
              {isProcessing ? <div className="w-4 h-4 rounded-full border-2 border-current border-t-transparent animate-spin" /> : <>Get OTP <ArrowRight className="w-4 h-4 ml-2" /></>}
            </button>
          </motion.form>
        )}
        ...
        {step === "otp" && (
          ...
          >
            ...
            <button type="submit" disabled={isProcessing || !otp} className={btnClass}>
              {isProcessing ? <div className="w-4 h-4 rounded-full border-2 border-current border-t-transparent animate-spin" /> : <>Verify OTP <ArrowRight className="w-4 h-4 ml-2" /></>}
            </button>
          </motion.form>
        )}
        ...
        {step === "pin" && (
          ...
          >
            ...
            <button type="submit" disabled={isProcessing || !pin} className={btnClass}>
              {isProcessing ? <div className="w-4 h-4 animate-spin rounded-full border-2 border-current border-t-transparent" /> : "Complete Login"}
            </button>
          </motion.form>
        )}
      </AnimatePresence>
