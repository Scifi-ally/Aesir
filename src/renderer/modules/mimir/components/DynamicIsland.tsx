...
import { FADE_SLOW } from "@/modules/mimir/lib/motion";
...
export function DynamicIsland() {
  ...
  const isOpen = Boolean(showCommandPalette || showIslandConfig || showEventFeed || islandConfig);
  const isExpanded = isOpen;
  const isModalOpen = Boolean(showCommandPalette || showIslandConfig || showEventFeed);
  ...
  return createPortal(
    <div className="isolate relative z-[1000] pointer-events-none">
      <AnimatePresence>
        {isModalOpen && (
          <motion.div
            initial={{ opacity: 0 }}
            ...
            className={`fixed inset-0 z-[9998] ${isNotification ? "pointer-events-none" : "pointer-events-auto bg-black/10"}`}
            onClick={() => {
              if (isNotification) {
                hideIsland();
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          <DynamicIslandBase
            placement="top"
            expanded={isExpanded}
            contentKey={showEventFeed ? "feed" : isSuccess ? "success" : isNotification ? "notification" : islandConfig ? "prompt" : showCommandPalette ? "palette" : "empty"}
            className="z-[9999]"
                ...
                <>
                  ...
                  {islandConfig?.onConfirm && (
                    <div className="flex w-full" style={{ marginTop: TOKENS.spacing.sectionGap, gap: TOKENS.spacing.buttonGap }}>
                      ...
                      >
                        {isProcessing ? <div className="w-4 h-4 text-[inherit] animate-spin rounded-full border-2 border-current border-t-transparent" /> : confirmText}
                      </button>
                    </div>
                  )}
                </>
