...
import { cn, fmtNum, toFixed, toFixedPct } from "@/modules/mimir/lib/format";
...
import type { PaperPosition } from "@/modules/mimir/types/api";
import { FADE_FAST, FADE_SLOW } from "@/modules/mimir/lib/motion";
                    ...
                    <>
                      {history.length > 0 && (
                        <motion.div variants={staggerItem} className="flex flex-wrap gap-6 py-2 border-b border-border/10 mb-2 text-[11px] font-mono text-foreground/50">
                          <span>Realized PnL: <span className={cn("font-normal", stats.totalRealizedPnl >= 0 ? "text-bull" : "text-bear")}>{stats.totalRealizedPnl > 0 ? '+' : stats.totalRealizedPnl < 0 ? '-' : ''}₹{fmtNum(Math.abs(stats.totalRealizedPnl), 2)}</span></span>
...
function PositionRow({ pos, onSelectSymbol, onClose }: { pos: PaperPosition; onSelectSymbol?: (symbol: string) => void; onClose?: () => void }) {
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-2 min-w-0">
        <div className="flex items-center gap-3">
          ...
          <span className={cn(
            ...
            pos.direction === "BUY" ? "text-bull" : "text-bear"
          )}>
            {pos.direction}
          </span>
          <span className="text-[10px] font-mono font-semibold tracking-[0.08em] uppercase text-primary">
            OPEN
          </span>
        </div>
        <div className="flex flex-wrap items-center gap-x-5 gap-y-1 text-[11px] font-mono font-normal text-foreground/50">
          ...
          {pos.trailingStopLoss && (
            <span className="flex items-center gap-1.5 text-accent">
              TSL <span className="font-normal">₹{fmtNum(Number(pos.trailingStopLoss), 2)}</span>
            </span>
          )}
          <span className="text-foreground/30">
      ...
      <div className="flex flex-col items-end gap-0.5 shrink-0">
        <span className={cn("text-sm font-mono font-normal tabular-nums", isProfit ? "text-bull" : isLoss ? "text-bear" : "text-foreground/40")}>
          {isProfit ? '+' : isLoss ? '-' : ''}₹<AnimatedNumber value={Math.abs(pnl)} decimals={2} duration={0.3} flashColor={true} />
        </span>
        <span className={cn("text-[10px] font-mono font-normal", isProfit ? "text-bull/70" : isLoss ? "text-bear/70" : "text-foreground/30")}>
          <AnimatedNumber value={pnlPct} decimals={2} showSign={true} suffix="%" duration={0.3} flashColor={true} />
        </span>
    ...
    >
      <div className="flex flex-col gap-2 min-w-0">
        <div className="flex items-center gap-3">
          ...
          <span className={cn(
            "text-[10px] font-mono font-semibold tracking-[0.08em] uppercase",
            hist.direction === "BUY" ? "text-bull" : "text-bear"
          )}>
            {hist.direction}
          </span>
          <span className={cn(
            "text-[10px] font-mono font-semibold tracking-[0.08em] uppercase",
            exitLabel.includes("TARGET") ? "text-bull" :
            exitLabel.includes("STOP") ? "text-bear" :
            "text-primary"
          )}>
            {exitLabel}
          </span>
      ...
      <div className="flex flex-col items-end gap-0.5 shrink-0">
        <span className={cn("text-sm font-mono font-normal tabular-nums", isProfit ? "text-bull" : isLoss ? "text-bear" : "text-foreground/40")}>
          {pnl > 0 ? "+" : pnl < 0 ? "-" : ""}₹{fmtNum(Math.abs(pnl), 2)}
        </span>
        <span className={cn("text-[10px] font-mono font-normal", isProfit ? "text-bull/70" : isLoss ? "text-bear/70" : "text-foreground/30")}>
          {toFixedPct(pnlPct, 2)}
        </span>
